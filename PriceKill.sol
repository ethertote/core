...
// Ethertote - CryptoPrice Game Kill 
// 31.07.18
...
contract CryptoPrice { function killGame() public pure {} }
...
contract PriceKill { 
    ...
    modifier remoteControl {
        require(
            ...
            msg.sender == processPayoutAddress ||
    ...
    constructor() public {   
        ...
        emit Deployed("Ethertote - PriceKill contract deployed:", block.timestamp);
    ...
    function killPriceContracts() onlyAdmin public {
        if (processPayoutAddress != 0x0) {processpayout.stopProcessPayout();}
        
        if (randomNumberAddress != 0x0) {random.stopRandomNumberCall(); }
            
        if (cryptoPriceAddress01 != 0x0) {cp01.killGame();}
        if (cryptoPriceAddress02 != 0x0) {cp02.killGame();}
        if (cryptoPriceAddress03 != 0x0) {cp03.killGame();}
        if (cryptoPriceAddress04 != 0x0) {cp04.killGame();}
		    if (cryptoPriceAddress05 != 0x0) {cp05.killGame();}
        if (cryptoPriceAddress06 != 0x0) {cp06.killGame();}
        if (cryptoPriceAddress07 != 0x0) {cp07.killGame();}
        if (cryptoPriceAddress08 != 0x0) {cp08.killGame();}
		    if (cryptoPriceAddress09 != 0x0) {cp09.killGame();}
        if (cryptoPriceAddress10 != 0x0) {cp10.killGame();}
		    if (cryptoPriceAddress11 != 0x0) {cp11.killGame();}
        if (cryptoPriceAddress12 != 0x0) {cp12.killGame();}
        if (cryptoPriceAddress13 != 0x0) {cp13.killGame();}
        if (cryptoPriceAddress14 != 0x0) {cp14.killGame();}
		    if (cryptoPriceAddress15 != 0x0) {cp15.killGame();}
        if (cryptoPriceAddress16 != 0x0) {cp16.killGame();}
        if (cryptoPriceAddress17 != 0x0) {cp17.killGame();}
        if (cryptoPriceAddress18 != 0x0) {cp18.killGame();}
		    if (cryptoPriceAddress19 != 0x0) {cp19.killGame();}
        if (cryptoPriceAddress20 != 0x0) {cp20.killGame();}
		    if (cryptoPriceAddress21 != 0x0) {cp21.killGame();}
        if (cryptoPriceAddress22 != 0x0) {cp22.killGame();}
        if (cryptoPriceAddress23 != 0x0) {cp23.killGame();}
        if (cryptoPriceAddress24 != 0x0) {cp24.killGame();}
		    if (cryptoPriceAddress25 != 0x0) {cp25.killGame();}

        emit ContractsStart("CryptoPrice contracts killed:", block.timestamp);
    }
